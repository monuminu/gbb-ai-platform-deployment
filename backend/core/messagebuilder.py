...
class MessageBuilder:
    ...
    def insert_message(self, role: str, content: Union[str, List[ChatCompletionContentPartParam]], index: int = 1):
        ...
        elif role == "function" and isinstance(content, str):
            message = ChatCompletionAssistantMessageParam(
                role="function", content=unicodedata.normalize("NFC", content))
        else:
            raise ValueError(f"Invalid role: {role}")

        self.messages.insert(index, message)

    def insert_func_message(self, role: str, content: str, func: dict, index: int = 1):
        """
        Inserts a message into the conversation at the specified index,
        or at index 1 (after system message) if no index is specified.
        Args:
            ...
            index (int): The index at which to insert the message.
        """
        self.messages.insert(index, {"role": role, "content": self.normalize_content(
            content), 'function_call': func})

    def insert_func_response_message(self, name: str, content: str, index: int = 1):
        """
        Inserts a message into the conversation at the specified index,
        or at index 1 (after system message) if no index is specified.
        Args:
            ...
            index (int): The index at which to insert the message.
        """
        self.messages.insert(index, {"role": "function", "content": self.normalize_content(
            content), 'name': name})
