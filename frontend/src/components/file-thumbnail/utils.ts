...
const FORMAT_IMG = ['jpg', 'JPG', 'jpeg', 'JPEG', 'gif', 'GIF', 'bmp', 'png', 'PNG', 'svg'];
...
export function fileFormat(fileUrl: string | undefined) {
  ...
  switch (fileUrl?.includes(fileTypeByUrl(fileUrl))) {
    case FORMAT_WEB.includes(fileTypeByUrl(fileUrl)):
      format = 'web';
      break;
    case FORMAT_MD.includes(fileTypeByUrl(fileUrl)):
      format = 'md';
      break;
...
export function fileTypeByUrl(fileUrl = '') {
  // if (fileUrl.startsWith('https://')) return 'https';
  // if (fileUrl.startsWith('http://')) return 'http';
