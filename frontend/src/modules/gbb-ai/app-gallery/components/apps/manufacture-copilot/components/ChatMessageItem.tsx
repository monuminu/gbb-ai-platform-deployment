...
import { getSources } from 'src/api/kmm';
...
export default function ChatMessageItem({
  ...
  const sourceFiles = useMemo(() => getSourceFiles(message.thoughts), [message.thoughts]);
    ...
    const fetchImageUrls = async () => {
      const imageUrls = await Promise.all(
        sourceFiles.map(async (item) => {
          const response = await getSources({ citation: item });
  ...
  const videoUrl = matchVid ? `/assets/mock/${matchVid[1].split('/').pop()}` : '';
  ...
  // if (message.thoughts && message.thoughts.length > 2)
  //   message.thoughts[2].description.forEach((item: any, index: number) =>
    ...
    <RootStyle>
      <Box sx={{ display: 'flex', ...(isMe && { ml: 'auto' }) }}>
        <div>
          ...
          >
            {isImage && <MessageImgStyle alt="attachment" src={message.body} onClick={() => {}} />}
            ...
            {isMe && message.sources !== undefined && message.sources.length > 0 && (
...
function getSourceFiles(thoughts: any[] | undefined) {
  if (!thoughts || thoughts.length < 3) {
    return [] as string[];
  }

  return [
    ...new Set(thoughts[2].description.map((thought: any) => thought.sourcefile)),
  ] as string[];
}
