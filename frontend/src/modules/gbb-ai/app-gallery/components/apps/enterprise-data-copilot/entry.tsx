...
export default function EnterpriseDataCopilot({ mlApp }: Props) {
  ...
  const [status, setStatus] = useState<'idle' | 'running'>('idle' as 'idle' | 'running');
  ...
  const [selectedIndex, setSelectedIndex] = useState(configurations.selectedIndex || '');
  ...
  const handleUpdateStatus = (newStatus: 'idle' | 'running') => {
    setStatus(newStatus);
  };
