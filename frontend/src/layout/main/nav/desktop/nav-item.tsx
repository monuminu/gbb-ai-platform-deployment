...
export const NavItem = forwardRef<HTMLDivElement, NavItemProps>(
  ({ title, path, open, active, hasChild, externalLink, subItem, ...other }, ref) => {
    ...
    return (
      <Link component={NavigationLink} path={path} color="inherit" underline="none">
        {renderContent}
      </Link>
    );
  }
...
export function NavItemDashboard({ path, sx, ...other }: NavItemDashboardProps) {
  return (
    <Link component={NavigationLink} path={path} sx={{ width: 1, height: 1 }} {...other}>
      <CardActionArea
        sx={{
          height: 1,
          minHeight: 320,
      ...
      >
        ...
        >
          <Box
            component="img"
            alt="auth_cover"
            src="/assets/drawing/auth_cover.png"
          />
        </m.div>
      </CardActionArea>
    </Link>
